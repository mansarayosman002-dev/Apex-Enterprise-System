...
export const QRCodesPage: React.FC = () => {
  ...
  const handleDownloadBadgePdf = (badge: EmployeeBadgeData) => {
    exportBadgeToPdf({
      fullName: badge.fullName,
      jobTitle: badge.jobTitle,
      department: badge.department,
      employeeId: badge.employeeId,
      employeeCode: badge.employeeCode,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 print:hidden">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">QR Badges</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Cryptographic QR credentials for terminal check-ins.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filtered.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="flex flex-col items-center justify-between rounded-2xl border-2 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-xs hover:border-indigo-400 dark:hover:border-indigo-600 hover:shadow-md transition text-center group max-w-sm mx-auto w-full"
              >
                ...
                <div className="w-full grid grid-cols-3 gap-1.5 pt-3 border-t border-slate-100 dark:border-slate-800 print:hidden mt-2">
                  <button
                    onClick={() => handleDownloadBadgePdf(badgeData)}
                    className="flex items-center justify-center space-x-1 rounded-lg border border-rose-200 dark:border-rose-900/60 bg-rose-50/80 dark:bg-rose-950/40 px-2 py-1.5 text-[11px] font-semibold text-rose-700 dark:text-rose-300 hover:bg-rose-100 dark:hover:bg-rose-900/60 transition shadow-2xs"
                    title="Save official employee ID badge as PDF"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center justify-center space-x-1 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-2 py-1.5 text-[11px] font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition shadow-2xs"
                  ...
                  <button
                    ...
                    className="flex items-center justify-center space-x-1 rounded-lg bg-indigo-50 dark:bg-indigo-950/60 px-2 py-1.5 text-[11px] font-semibold text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 transition shadow-2xs"
